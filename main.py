...
def main():
    ...
    for rounds in range(ROUNDS):
        seed(7)
        for stupid in range(START, MAX_POINTS, INC_BY):
            poly = generate_polygon(stupid)
            ...
            if rounds == 0 and SHOW_PLOT:
                ...
                for seg in timer(ear_clip, poly, time_map=EAR_MAP, iter=stupid):
                    a_x, a_y = seg.a.x, seg.a.y
                    b_x, b_y = seg.b.x, seg.b.y
                    plt.plot([a_x, b_x], [a_y, b_y])
                plt.show()
            else:
                _ = timer(ear_clip, poly, time_map=EAR_MAP, iter=stupid)
    ...
    for rounds in range(ROUNDS):
        ...
        for stupid in range(START, MAX_POINTS, INC_BY):
            ...
            if rounds == 0 and SHOW_PLOT:
                ...
                for seg in timer(polygon_decomposition, poly, time_map=CONCAVE_MAP, iter=stupid):
                    a_x, a_y = seg.a.x, seg.a.y
                    b_x, b_y = seg.b.x, seg.b.y
                    plt.plot([a_x, b_x], [a_y, b_y])
                plt.show()
            else:
                _ = timer(polygon_decomposition, poly, time_map=CONCAVE_MAP, iter=stupid)
