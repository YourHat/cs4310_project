...
def neighbors(origin, seg) -> bool:
    ...
    if seg.a.x == origin.a.x:
        #print("d1")
        return True
    elif seg.a.y == origin.b.y:
        #print("d2")
        return True
    elif seg.b.x == origin.a.x:
        #print("d3")
        return True
    elif seg.b.x == origin.b.x:
        #print("d4")
        return True
    else:
        return False
...
def clip(polygons) -> list:
    ...
    global triangles, seg_list
    triangles = []
    seg_list = [] 
    polygon = polygons.segs 
    lin = LineSegment(polygon[-1].b, polygon[0].a)
    polygon.append(lin)
    ...
    while(len(polygon) > 4): #stop when there are three line segments left -> then create a triangle from the lines

     
        for seg in polygon:
            if not (((seg.a.x == seg_next.a.x) and (seg.b.x == seg_next.b.x)) or ((seg.b.x == seg_next.a.x) and (seg.a.x == seg_next.b.x))):
                #print(seg, seg_next)
                if neighbors(seg, seg_next):
                    if not (seg.a.x== seg_current.a.x or seg.b.x == seg_current.b.x or seg.a.x == seg_current.b.x or seg.b.x == seg_current.a.x): 
    ...
    #del polygon[0]
    #del polygon[1]
    #make_triangle(polygon[2],polygon[3],triangles)
    #return triangles
    #print("done")
    return seg_list
